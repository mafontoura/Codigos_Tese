...
function preproc(window_name){
	...
	run("Create Mask");
	mask_placenta = getTitle(); // criando uma mascara para aplicar para o SLC tambem 
	...
	// processando apenas o SLC 
	selectWindow("SLC");
	run("Duplicate...", " ");
	rename("SLC_positive");

	// deixando o usuario escolher qual eh o melhor threshold para SLC positivas 
	run("Threshold...");
	...
	run("Convert to Mask");	
	SLC_threshold = getTitle();
	
	imageCalculator("Multiply create", mask_placenta, SLC_threshold);
	run("Convert to Mask");	
	SLC_positive = getTitle();
	
	// analisando todas as celulas em relacao a intensidade do SLC	
	selectWindow(all_threshold);
	run("Duplicate...", "title=DAPI_all-cells");
	selectWindow("DAPI_all-cells");
	run("Set Measurements...", "area mean shape area_fraction limit display redirect=SLC decimal=4");
	...
	//agora analisando a intensidade do SLC apenas em celulas positivas (acima do threshold) 
	selectWindow(all_threshold);
	//run("Morphological Filters", "operation=Dilation element=Disk radius=1");
	//imageCalculator("AND create", SLC_positive , all_threshold + "-Dilation");
	imageCalculator("AND create", SLC_positive , all_threshold);
	SLC_positive_inside_cells = getTitle();
	
	selectWindow(SLC_positive_inside_cells);
	...
	wsd = getTitle();
	run("Intensity Measurements 2D/3D", "input=SLC labels=" + wsd +" mean stddev max min median mode skewness kurtosis numberofvoxels volume neighborsmean neighborsstddev neighborsmax neighborsmin neighborsmedian neighborsmode neighborsskewness neighborskurtosis");
	
	print("NAME:  " + dir + "/" + outputdir +"/table_results/" );
	saveAs("Results",dir + "/" + outputdir +"/table_results/" + "SLC-positive_cells_" + name +"_T-" + T + ".csv");

	selectWindow(all_threshold);
	saveAs("Tiff", dir+"/"+outputdir +"/" + name + "_DAPI-all-cells.tif");
	result = getTitle();
	
	selectWindow(SLC_positive);
	saveAs("Tiff", dir+"/"+outputdir +"/" + name + "_SLC-positive.tif");
	result = getTitle();
	
	selectWindow(wsd);
	saveAs("Tiff",dir+"/"+outputdir +"/" +  name + "_SLC-watershed.tif");
	all_threshold = getTitle();
