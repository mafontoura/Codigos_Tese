macro "Analisar imagens de EPCAM" {
...
function preproc(window_name){
	...
	run("Create Mask");
	mask_placenta = getTitle(); // criando uma mascara para aplicar para o EPCAM tambem 
	...
	// processando apenas o EPCAM 
	selectWindow("EPCAM");
	run("Duplicate...", " ");
	rename("EPCAM_positive");

	// deixando o usuario escolher qual eh o melhor threshold para EPCAM positivas 
	run("Threshold...");
	waitForUser("adjust threshold","Click OK when you are done");
	
	Dialog.create("select threshold");
	Dialog.addSlider("Threshold value", 0, 255, 39);
	Dialog.show();
	...
	run("Convert to Mask");	
	EPCAM_threshold = getTitle();
	
	imageCalculator("Multiply create", mask_placenta, EPCAM_threshold);
	run("Convert to Mask");	
	EPCAM_positive = getTitle();
	
	// analisando todas as celulas em relacao a intensidade do EPCAM	
	selectWindow(all_threshold);
	run("Duplicate...", "title=DAPI_all-cells");
	selectWindow("DAPI_all-cells");
	run("Set Measurements...", "area mean shape area_fraction limit display redirect=EPCAM decimal=4");
	...
	//agora analisando a intensidade do EPCAM apenas em celulas positivas (acima do threshold) 
	selectWindow(all_threshold);
	run("Morphological Filters", "operation=Dilation element=Disk radius=1");
	imageCalculator("AND create", EPCAM_positive , all_threshold + "-Dilation");
	//imageCalculator("AND create", EPCAM_positive , all_threshold);
	EPCAM_positive_inside_cells = getTitle();
	
	selectWindow(EPCAM_positive_inside_cells);
	...
	wsd = getTitle();
	run("Intensity Measurements 2D/3D", "input=EPCAM labels=" + wsd +" mean stddev max min median mode skewness kurtosis numberofvoxels volume neighborsmean neighborsstddev neighborsmax neighborsmin neighborsmedian neighborsmode neighborsskewness neighborskurtosis");
	
	print("NAME:  " + dir + "/" + outputdir +"/table_results/" );
	saveAs("Results",dir + "/" + outputdir +"/table_results/" + "EPCAM-positive_cells_" + name +"_T-" + T + ".csv");

	selectWindow(all_threshold);
	saveAs("Tiff", dir+"/"+outputdir +"/" + name + "_DAPI-all-cells.tif");
	result = getTitle();
	
	selectWindow(EPCAM_positive);
	saveAs("Tiff", dir+"/"+outputdir +"/" + name + "_EPCAM-positive.tif");
	result = getTitle();
	
	selectWindow(wsd);
	saveAs("Tiff",dir+"/"+outputdir +"/" +  name + "_EPCAM-watershed.tif");
	all_threshold = getTitle();
